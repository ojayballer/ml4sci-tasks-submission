...
def train_one_epoch(train_loader,encoder,classifier,device,optimizer,criterion,e,epochs,scheduler):
    ...
    for _,(image,labels) in loop:
        ...
        try:
            ...
            scores1.update(auc, batch_size)
            scores2.update(accuracy, batch_size)
            ...
            loop.set_postfix(loss = loss.item(), auc = auc, acc = accuracy ,stage = 'train')
...
def val_one_epoch(loader,encoder,classifier,device,criterion):
    ...
    for _,(image,labels) in loop:
        ...
        try:
            ...
            scores1.update(auc, batch_size)
            scores2.update(accuracy, batch_size)
            loop.set_postfix(loss = loss.item(), auc = auc, acc = accuracy ,stage = 'val')
            global_step += 1
        except ValueError:
            pass
